                  ...
                  vtkImageData *data, vtkPolyData *output, T *ptr, bool outputTriangles)
{
  ...
  vtkThreadedSynchronizedTemplatesCutter3DInitializeOutput(exExt, self->GetOutputPointsPrecision(), data, output);
...
public:
  ...
  DoThreadedCut(vtkThreadedSynchronizedTemplatesCutter3D* filter,
    int* exExt, vtkImageData* input, int numPieces) :
                     Filter(filter), ExExt(exExt), Input(input),
                     NumberOfPieces(numPieces)
  {
    this->Outputs = new vtkPolyData*[numPieces];
    this->Plane = vtkPlane::SafeDownCast(filter->GetCutFunction());
    if (this->Plane)
      {
      this->Origin = this->Plane->GetOrigin();
      this->Normal = this->Plane->GetNormal();
      this->ImageOrigin = input->GetOrigin();
      this->Spacing = input->GetSpacing();
      }
  }
  ...
  inline char SideOfPlane(int* indices)
  {
    double pt0[3];
    double delta[3];
    for (int i=0; i<3; i++)
      {
      pt0[i] = this->ImageOrigin[i] + indices[i]*this->Spacing[i];
  ...
  bool ShouldDoCut(int* exExt)
  {
    ...
    int pointIndices[24] = {exExt[0], exExt[2], exExt[4],     // 0, 0, 0
                            exExt[1], exExt[2], exExt[4],     // 1, 0, 0
                            exExt[0], exExt[3], exExt[4],     // 0, 1, 0
                            exExt[0], exExt[2], exExt[5],     // 0, 0, 1
                            exExt[1], exExt[3], exExt[4],     // 1, 1, 0
                            exExt[0], exExt[3], exExt[5],     // 0, 1, 1
                            exExt[1], exExt[2], exExt[5],     // 1, 0, 1
                            exExt[1], exExt[3], exExt[5] };   // 1, 1, 1
    char total = 0;
    for (int i=0; i<8; i++)
      {
      total += this->SideOfPlane(&pointIndices[i*3]);
      }
  ...
  void operator()( vtkIdType begin, vtkIdType end )
  {
    ...
    for(int i=begin; i<end; i++)
      ...
      et->PieceToExtent();
      et->GetExtent(exExt2);
      if (this->ShouldDoCut(exExt2))
        {
        ContourImage(this->Filter, exExt2, this->Input, pdata,
                     (double*)0, true);
        }
      this->Outputs[i] = pdata;
      }
  }
};
...
//
void vtkThreadedSynchronizedTemplatesCutter3D::ThreadedExecute(vtkImageData *data,
                                                       ...
                                                       int)
{
  ...
  for(int i=0; i<nPieces; i++)
    {
    vtkPolyData* contour = functor.Outputs[i];
